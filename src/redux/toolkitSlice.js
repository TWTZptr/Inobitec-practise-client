...
export const removeSelectedNode = createAsyncThunk('nodes/remove', async (nodeId, thunkAPI) => {
    const selectedNodeId = thunkAPI.getState().ui.selectedNode.id;
    const response = await Api.removeNode(selectedNodeId);
...
const nodes = createSlice({
    ...
    extraReducers: (builder) => {
        builder
            ...
            .addCase(removeSelectedNode.fulfilled, (state, {payload}) => {
                const indexToDelete = state.findIndex(item => item.id === payload);
                state.splice(indexToDelete, 1);
            })
            .addCase(removeSelectedNode.rejected, (state, {payload}) => {
...
const ui = createSlice({
    ...
    reducers: {
        ...
        resetSelect: (state, action) => {
            ...
            state.selectedNode.parent_id = null;
        }
    }
});
